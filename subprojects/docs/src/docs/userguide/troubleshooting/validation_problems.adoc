...
This error indicates that a task declares an input property with values of an unsupported type.

To fix this, look at the error message which indicates the list of possible solutions.
...
== Type is unsuited for nested annotation

This error indicates that an unsuited type is annotated as nested.
The nested annotation marks a property as specifying a nested bean, whose properties should be checked for annotations.
Primitive wrapper types and other types like String and File do not represent a nested bean, which makes them unsuited for the annotation.

To fix this problem, change to a suited type, e.g. `Provider<T>`, `Iterable<T>` or <MapProperty<K, V>>.
